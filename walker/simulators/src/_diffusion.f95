!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE RUN(X, mu, icov, A, dt, friction, kT, n_features, n_frames, n_peaks)
    IMPLICIT NONE
    INTEGER(kind=4),INTENT(in) :: n_features, n_frames, n_peaks
    DOUBLE PRECISION,INTENT(in) :: dt, friction, kT
    ...
    DO frame=1,n_frames-1
        ...
        DO k=1,n_features
            ...
            F_random(k) = DSQRT((2*kT*dt)/friction)*DSIN(2.0E+00*pi*randNum)
        END DO
